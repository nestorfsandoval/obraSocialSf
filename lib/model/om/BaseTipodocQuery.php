...
/**
 * Base class that represents a query for the 'tipodoc' table.
 *
 * 
 *
 ...
 *
 * @method     TipodocQuery leftJoinAfiliado($relationAlias = null) Adds a LEFT JOIN clause to the query using the Afiliado relation
 * @method     TipodocQuery rightJoinAfiliado($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Afiliado relation
 * @method     TipodocQuery innerJoinAfiliado($relationAlias = null) Adds a INNER JOIN clause to the query using the Afiliado relation
 *
 ...
 *
 * @method     Tipodoc findOneById(int $id) Return the first Tipodoc filtered by the id column
...
abstract class BaseTipodocQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `DESCRIPCION` FROM `tipodoc` WHERE `ID` = :p0';
        try {
    ...
    public function filterByAfiliado($afiliado, $comparison = null)
    {
        if ($afiliado instanceof Afiliado) {
            return $this
                ->addUsingAlias(TipodocPeer::ID, $afiliado->getTipodocId(), $comparison);
